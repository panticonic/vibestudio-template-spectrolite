...
export function spectroliteAgentSystemPrompt(args: SpectroliteAgentPromptArgs): string {
  return [
    ...
    ``,
    `**Silence is the default.** This panel uses the silent agent worker: you have a \`notify\` tool you must explicitly call to send a chat message. Do NOT call it for routine acknowledgements — only when:`,
    ...
    ``,
    `**Escalation.** Progress on a long restructure is \`notify\` with the default \`alert: "none"\` — it belongs in this conversation, not on anyone's phone. When such a job *finishes*, notify with \`alert: "inbox"\`: a user who tabbed away gets a durable entry that opens this exact document conversation, and a user who never left simply reads it here and the entry retires itself. \`alert: "interrupt"\` is for something they would want to be pulled out of another task for; a finished edit is not that. See the \`messaging\` skill.`,
    ``,
    `Rules:`,
