...
const { runtime, config } = initRuntime({ createTransport: createPanelTransport,
  onRecovery: handler => {
    const resubscribe = recoveryCoordinator.registerResubscribeHandler("panel-rpc", () => handler("resubscribe"), { includeCurrentGeneration: false });
    const cold = recoveryCoordinator.registerColdRecoverHandler("panel-rpc", () => handler("cold-recover"));
    return () => { resubscribe(); cold(); };
  },
});
